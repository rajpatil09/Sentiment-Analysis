...
for file in files:
	if(os.path.isdir(os.path.join(filepath,file)) == False):
		...
		writefile = open(filepath+"Senti/Senti_"+renamefile+"_Data.txt", "a+")
		cnt=0
		print(filepath+file)
...
for line in open(filepath+'Senti/Senti_train_Data.txt', 'r'):
	word_tokens = word_tokenize(line)
	all_words.append(word_tokens)
	#all_data.append(line)

for line in open(filepath+'Senti/Senti_test_Data.txt', 'r'):
	word_tokens = word_tokenize(line)
	all_words.append(word_tokens)
	#all_data.append(line)
...
save_features = open("Amazon/Senti/Senti_Features.pickle","wb")
pickle.dump(word_features, save_features)
save_features.close()

save_docs = open("Amazon/Senti/Senti_DocsLabels.pickle","wb")
pickle.dump(documents, save_docs)
save_docs.close()


'''
getFeatures = open("Amazon/Senti/Senti_Features.pickle", "rb")
word_features = pickle.load(getFeatures)
getFeatures.close()

getDocs = open("Amazon/Senti/Senti_DocsLabels.pickle", "rb")
